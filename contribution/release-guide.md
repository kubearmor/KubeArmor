...
Overview
KubeArmor publishes release artifacts that are signed with Sigstore cosign via GoReleaser. Starting with cosign v3, the signer must explicitly choose bundle behavior. KubeArmor signs with --bundle=false, which matches our previous behavior: the signature and the signing certificate are published alongside each artifact. Use them to verify what you download.

Prerequisites
- cosign installed (v2.2+ or v3.x)
- sha256sum or shasum available in your shell
