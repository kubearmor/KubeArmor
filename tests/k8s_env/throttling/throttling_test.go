...
var _ = Describe("Smoke", func() {
	...
	Describe("Alert Throttling", func() {
		...
		It("enabled with new throttling conditions", func() {
			...
			AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "count=0; while [ $count -lt 5 ]; do apt; count=$((count + 1)); done;"}, MatchRegexp("apt.*Permission denied"), false)
			...
			//fmt.Printf("---START---\n%s---END---\n", sout)
			//Expect(sout).To(MatchRegexp("apt.*Permission denied"))
			AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "apt update"}, MatchRegexp("apt.*Permission denied"), false)
