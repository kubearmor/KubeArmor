		...
		It("can block execution of pkg mgmt tools such as apt, apt-get", func() {
			...
			Expect(alerts[0].Severity).To(Equal("3"))
				...
				It("allow access for service account token to only cat", func() {
					...
					AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "cat /run/secrets/kubernetes.io/serviceaccount/token"}, Not(ContainSubstring("Permission denied")), false)
					...
					AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "cat /etc/passwd"}, Not(ContainSubstring("Permission denied")), false)
					...
					AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "head /etc/passwd"}, Not(ContainSubstring("Permission denied")), false)
				...
				It("can block write access and only allow read access to mounted files", func() {
					...
					AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "touch /dev/shm/new"}, ContainSubstring("Permission denied"), true)
				...
				It("will allow use of tcp network protocol by curl and bash", func() {
					...
					AssertCommand(wp, "wordpress-mysql", []string{"bash", "-c", "curl 142.250.193.46"}, ContainSubstring("http://www.google.com"), true )
					...
					[]string{"bash", "-c", "curl google.com"}, ContainSubstring("http://www.google.com/"), true)
