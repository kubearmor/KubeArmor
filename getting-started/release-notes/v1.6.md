...
{
  "🐜 Bug Fixes": [
    "TCP connection monitoring now reports the correct network protocol name instead of a corrupted or garbled value in event output. In the `kretprobe__tcp_connect` eBPF handler, the protocol string is read from `skc_prot-\u003ename` using `READ_KERN`, copied into a temporary stack buffer with `bpf_probe_read_str`, and then stored into the event arguments (Fixes #2353, commit 3382dc81cebd87f6564cf451cb56a7f5347dd1d0)."
  ]
}
