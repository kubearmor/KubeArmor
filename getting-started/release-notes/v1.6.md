...
- Improved how the system records process execution events so entries with incomplete process details are skipped instead of producing confusing logs.

### 🐜 Bug Fixes

- Updated the KubeArmor Helm chart so installations render correctly and workloads start with the intended settings.
- Reduced overhead and improved correctness for TCP accept event processing so it behaves consistently with TCP connect tracing.

### 🎁 Misc

- Disabled monitoring of the accept system call in the system monitor to reduce unnecessary event capture.
- Bumped module dependencies to Go 1.24.12 to keep builds consistent and up to date.
