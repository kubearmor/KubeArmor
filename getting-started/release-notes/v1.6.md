...
- Improved process execution event logging by skipping entries with incomplete process details.

### 🐜 Bug Fixes

- Fixed the KubeArmor Helm chart templating so installs render correctly and workloads start with the intended settings.
- Improved TCP accept event processing to reduce overhead and align behavior with TCP connect tracing.

### 🎁 Misc

- Disabled `accept` syscall monitoring in the system monitor to reduce unnecessary event capture.
- Updated Go module versions from 1.24.11 to 1.24.12.
