...
{
  "🐜 Bug Fixes": [
    "Corrected how the CRI socket address is interpreted so the runtime connection works reliably in more setups. Specifically, the CRI socket URL formatting is fixed and the command-line args slice is now updated in place to ensure the corrected socket argument is actually applied."
  ]
}
