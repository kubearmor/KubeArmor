...
- **Title:** Stability and maintenance update
- **Date:** 2026-02-22
- **Version:** v1.6.8
...
{
  "🆕 New Features": [
    "You can now configure KubeArmor with additional custom settings when deploying it using Helm. This adds support for passing custom KubeArmor arguments through Helm `values.yaml` (for example via a new/expanded args-style values field that is rendered into the KubeArmor container command-line)."
  ],
  "🎁 Misc": [
    "The Helm chart is now aligned with the latest KubeArmor release so installs and upgrades use the expected versions. This updates the chart and application versions from v1.6.7 to v1.6.8 in the Helm chart metadata.",
    "The Helm chart configuration was cleaned up to avoid unnecessary formatting noise in the repository. This removes trailing whitespace on an empty line in the KubeArmor Helm `values.yaml` file."
  ],
  "🐜 Bug Fixes": [
    "Deployments on k3s now mount the container runtime socket from the correct location so KubeArmor can connect reliably. This updates the Helm chart’s containerd socket volume mount path to the k3s-specific socket path used by containerd on k3s."
  ]
}
