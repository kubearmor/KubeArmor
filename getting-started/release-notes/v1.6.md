...
{
  "🎁 Misc": [
    "Some low-value system monitoring was removed to reduce noise and potential overhead in runtime observation.",
    "The project’s Go module dependencies were refreshed to stay current with the latest patch release.",
    "Process event logging now avoids producing incomplete records when key process details are missing."
  ],
  "🐜 Bug Fixes": [
    "Helm chart deployment settings for KubeArmor were corrected so installs and upgrades behave more reliably across clusters.",
    "TCP accept telemetry was made safer and more consistent so it is less likely to misreport protocol information under load."
  ]
}
