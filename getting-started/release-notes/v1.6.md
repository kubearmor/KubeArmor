...
- Clarified RHEL 9 build gating by defining `RHEL9_BUILD_GTE_400` only when the RHEL9 build number is >= 400.
- Reduced security scanning noise by renaming a Kubernetes secret field and adding gosec suppression comments around specific file checks and HTTP calls.
