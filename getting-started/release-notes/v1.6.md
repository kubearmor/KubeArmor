...
{
  "🎁 Misc": [
    "TCP accept handling now uses a more memory-efficient approach so it consumes less stack space while staying consistent with related connection tracking. Specifically, `tcp_accept` now uses a perf-buffer-backed context and a safe fixed-size protocol string, aligned with the `tcp_connect` implementation.",
    "Protocol detection is now more reliable on systems that support modern kernel type information, while still working on older builds. Specifically, when BTF is available the code reads `sk_protocol` directly, and it falls back to the offset-based read only for non-BTF builds.",
    "The project’s Go module dependencies were refreshed to the latest patch level to keep builds current. Specifically, all module `go.mod` files were updated from Go 1.24.11 to Go 1.24.12."
  ],
  "🐜 Bug Fixes": [
    "The system monitor no longer tracks new incoming connections via the accept system call, reducing monitoring noise and overhead. Specifically, monitoring for `accept(2)` has been disabled in the system monitor."
  ]
}
