...
## KubeArmor v1.6.6 (2026-01-28)

Patch release for KubeArmor v1.6 with stability, compatibility, and configuration improvements.

📄 [Full CHANGELOG for v1.6.6](https://github.com/kubearmor/KubeArmor/releases/tag/v1.6.6)

### 🆕 New Features

- Added configurable process-argument matching so policies can allow or restrict commands based on their runtime arguments.
- Added configuration options to control visibility behavior at runtime, including namespace tracking and log verbosity.
- Added safer cleanup behavior to prevent accidental deletion of critical paths during runtime cleanup.
- Added the ability to explicitly disable LSM enforcement via configuration and propagate that intent through deployment options.

### 🐜 Bug Fixes

- Fixed unsafe handling of protocol name strings in the `tcp_connect` path to avoid invalid memory access in BPF programs.
- Fixed enforcement/key-size mismatches by tightening path and argument length limits to align CRDs and BPF map keys.

### 🎁 Misc

- Improved codebase maintainability by modernizing Go usage and removing deprecated or unused constructs without changing behavior.
- Updated CI and tests to be more reliable and consistent across environments.
- Improved kernel/platform compatibility by narrowing special BPF compilation paths to the intended targets.
- Updated documentation to clarify supported environments and usage examples.
- Updated security scanning and release packaging metadata to current versions.
- Updated repository headers to reflect the current year without functional changes.
- Refactored CLI/operator option handling to reduce duplication and keep configuration consistent across components.
