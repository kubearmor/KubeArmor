...
- Added consolidated operator configuration using **OperatorOptions** and passed the selected LSM order to managed KubeArmor components via command-line arguments.

### 🐛 Bug Fixes

- Fixed LSM detection when `lsmOrder: none` is set by skipping detection and treating unsupported LSMs as `none`.

### 🎁 Misc

- Simplified snitch configuration by introducing **SnitchOptions** and removing global variables.
