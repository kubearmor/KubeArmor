...
{
  "🆕 New Features": [
    "You can now match process executions based on their command-line arguments, so policies can allow or deny a program more precisely than by path alone."
  ],
  "🎁 Misc": [
    "The system uses less memory for certain internal keys while keeping schema validation aligned with runtime constraints."
  ],
  "🐜 Bug Fixes": [
    "Policies that use argument matching now enforce ownership and argument constraints more reliably and clean up state more safely."
  ]
}
```
