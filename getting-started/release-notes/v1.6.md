...
- Added cached username resolution so host activity is easier to interpret in day-to-day operations.
- Added an option to explicitly select the container runtime socket so deployments can target the correct CRI endpoint.

### 🐜 Bug Fixes

- Fixed race conditions that could occur during concurrent endpoint and host policy updates.
- Fixed alert throttling concurrency issues so alerts are tracked safely under load.
- Fixed runtime detection so it consistently reports the correct runtime derived from the detected socket.
- Fixed the RHEL9 build macro gating so the compatibility define is only applied when appropriate.

### 🎁 Misc

- Improved workload reliability defaults by setting resource sizing and ensuring required runtime configuration is always present.
- Improved test and CI configurability by aligning Helm keys and passing runtime ordering through the test harness.
- Improved the stability and runtime of race-condition test coverage.
- Updated build and dependency versions to keep the project current and compatible.
- Improved host-only runs and USB log readability for easier local troubleshooting.
- Updated configuration and security-scanner hygiene around Kubernetes secret naming and static analysis findings.
