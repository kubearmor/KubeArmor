...
{
  "🆕 New Features": [
    "Added consolidated operator configuration via an `OperatorOptions` struct, replacing scattered settings and passing the selected LSM order to managed KubeArmor components via command-line arguments."
  ],
  "🎁 Misc": [
    "Simplified snitch configuration by introducing a unified `SnitchOptions` struct and removing multiple global variables."
  ],
  "🐜 Bug Fixes": [
    "Fixed LSM detection for `lsmOrder: none` by skipping detection and treating unsupported LSMs as `none`."
  ]
}
