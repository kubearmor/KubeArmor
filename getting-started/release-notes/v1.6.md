...
KubeArmor v1.6.8 (2026-02-22) is a stability and maintenance update.

### 🆕 New Features

- Added Helm chart support for passing custom KubeArmor arguments via `values.yaml`.

### 🐜 Bug Fixes

- Fixed the Helm chart containerd socket volume mount path on k3s to use the correct k3s containerd socket.

### 🎁 Misc

- Aligned the Helm chart metadata with KubeArmor v1.6.8 (chart/app versions updated from v1.6.7).
- Removed trailing whitespace from the KubeArmor Helm `values.yaml` file.
