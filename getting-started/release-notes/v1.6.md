...
## KubeArmor v1.6.6 Patch Release

**Title:** v1.6.6 Patch Release

**Date:** 2026-02-04

**Version:** v1.6.6

<!-- BEGIN AUTO-GENERATED RELEASE NOTES -->
{
  "🎁 Misc": [
    "Some low-value system monitoring was removed to reduce noise and potential overhead in runtime observation. Specifically, monitoring of the `accept` system call was disabled in the system monitor.",
    "The project’s Go module dependencies were refreshed to stay current with the latest patch release. All module `go.mod` files were updated from Go 1.24.11 to Go 1.24.12.",
    "Process event logging now avoids producing incomplete records when key process details are missing. Process-name / process-information validation was centralized and applied to syscall exec/`execve` event logging so events with missing process information are skipped."
  ],
  "🐜 Bug Fixes": [
    "Helm chart deployment settings for KubeArmor were corrected so installs and upgrades behave more reliably across clusters. The KubeArmor Helm chart was updated and the workloads YAML was fixed, including CLI/TLS flags, `imagePullSecrets`, and tolerations handling (using the latest 2026-01-30 state over earlier drafts).",
    "TCP accept telemetry was made safer and more consistent so it is less likely to misreport protocol information under load. The tcp_accept eBPF path now uses a perf-buffer-backed context with a safe fixed-size protocol string, and it prefers direct `sk_protocol` access when BTF is available while keeping an offset-based fallback for non-BTF builds."
  ]
}
