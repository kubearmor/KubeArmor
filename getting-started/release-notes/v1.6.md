...
Released on **2026-02-04** (version **v1.6.6**).

### 🆕 New Features

- Added maintenance-only updates in this patch release (no new user-facing features).

### 🐜 Bug Fixes

- Fixed Helm chart deployment settings for KubeArmor so installs and upgrades work more reliably across clusters.
- Fixed TCP accept telemetry so it is safer and less likely to misreport protocol information.

### 🎁 Misc

- Removed low-value system monitoring to reduce noise and potential overhead in runtime observation.
- Updated Go module dependencies to stay current with the latest patch release.
- Improved process event logging to avoid producing incomplete records when key process details are missing.
