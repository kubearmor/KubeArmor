...
{
  "🐛 Bug Fixes": [
    "We fixed an issue where the container runtime interface (CRI) socket address could be formatted incorrectly, which could prevent the system from connecting to the runtime. Specifically, we corrected the CRI socket URL formatting logic and ensured the arguments slice is updated in place so downstream consumers see the corrected value."
  ]
}
