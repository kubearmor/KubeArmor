...
```jsonc
{
  ...
  "🎁 Misc": [
    "CI workflows now use the correct Helm values key for the kubearmor-operator image tag. The pipeline configuration was updated to match the operator chart’s image tag key to keep automated builds and tests aligned.",
...
- Fixed data races in alert throttling by protecting AlertMap with an RWMutex.
...
- Fixed RHEL9 build gating so `RHEL9_BUILD_GTE_400` is defined only when the build number is >= 400.
...
- Updated dependencies and release metadata, including Go 1.24.13, `github.com/cilium/cilium` v1.18.4, and the stable release version to v1.6.8.
- Reduced static analysis noise by renaming a Kubernetes secret field and adding targeted gosec suppressions.
