...
{
  "🎁 Misc": [
    "Improved the reliability and speed of the concurrency test suite to reduce flakiness during automated runs. The race-condition test was shortened, its timing behavior was simplified by removing per-iteration sleeps, and it was updated with minor formatting fixes and a refreshed copyright year."
  ],
  "🐜 Bug Fixes": [
    "Improved the system’s thread-safety when multiple operations access endpoint and host policy data at the same time. It now avoids race conditions by copying shared slices under read locks, applying updates under full locks, and validating the behavior with updated race-condition tests that randomly shrink the endpoints slice and run faster without per-iteration sleeps."
  ]
}
