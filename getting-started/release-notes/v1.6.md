...
{
  "🐜 Bug Fixes": [
    "Fixed untracked namespace configuration changes so updates take effect while KubeArmor is running instead of only after a restart. The `kubearmor-config` ConfigMap watcher now refreshes the **untrackedNs** value via `cfg.GlobalCfg.ConfigUntrackedNs` (backed by `sync/atomic.Value`) so policy enforcement, visibility handling, and syscall tracing use the updated list (fixes #2315; final commit a827a532d79986cd4733c89cc15666c355b3faa6)."
  ]
}
