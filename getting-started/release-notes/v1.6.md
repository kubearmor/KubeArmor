...
{
  "🆕 New Features": [
    "The system can now skip starting the enforcer automatically when no Linux Security Module (LSM) is configured, which avoids unnecessary setup and potential startup problems in environments that don’t use an LSM. Specifically, initialization is now guarded by a configuration-based check so the enforcer is not initialized unless an LSM is explicitly specified."
  ]
}
