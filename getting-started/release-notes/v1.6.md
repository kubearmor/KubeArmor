...
{
  "🆕 New Features": [
    "Added cached username resolution so host activity is easier to interpret in day-to-day operations. The agent now caches UID-to-username lookups and propagates usernames through internal types and protobuf APIs into host logs and alert payloads.",
    "Added an option to explicitly select the container runtime socket so deployments can target the correct CRI endpoint. Helm now plumbs a `--socket-file` setting through to **snitch**, validates the socket path, and includes tests for the selection logic."
  ],
  "🎁 Misc": [
    "Improved workload reliability defaults by setting resource sizing and ensuring required runtime configuration is always present. Multiple manifests now include Kubernetes resource requests/limits, and the generated DaemonSet always sets the CRI socket argument.",
    "Improved test and CI configurability by aligning Helm keys and passing runtime ordering through the test harness. CI workflows now use the updated Helm value key for the `kubearmor-operator` image tag, and tests forward an LSM order flag into KubeArmor execution.",
    "Improved the stability and runtime of race-condition test coverage. The race tests now randomize endpoint removal, shorten overall duration, and include minor structural/formatting cleanups.",
    "Updated build and dependency versions to keep the project current and compatible. The stable release version is now `v1.6.8`, Go is updated to `1.24.13`, and `github.com/cilium/cilium` is bumped to `v1.18.4`.",
    "Improved host-only runs and USB log readability for easier local troubleshooting. The host-only run target now sets `k8s=false`, and USB logs include timestamps.",
    "Updated configuration and security-scanner hygiene around Kubernetes secret naming and static analysis findings. A Kubernetes secret field was renamed and gosec suppression comments were added around specific file checks and HTTP calls where the behavior is intentional."
  ],
  "🐜 Bug Fixes": [
    "Fixed race conditions that could occur during concurrent endpoint and host policy updates. The code now copies slices under read locks, updates originals via keyed lookup under write locks, adds missing endpoint locking on a containerd update path, and includes race-condition test coverage for these flows.",
    "Fixed alert throttling concurrency issues so alerts are tracked safely under load. `AlertMap` is now protected by an `RWMutex` that is used consistently across alert-throttling methods.",
    "Fixed runtime detection so it consistently reports the correct runtime derived from the detected socket. `DetectRuntimeViaMap` now calls `determineRuntimeFromSocket` on the matched socket-file path before returning.",
    "Fixed the RHEL9 build macro gating so the compatibility define is only applied when appropriate. `RHEL9_BUILD_GTE_400` is now defined only when the RHEL9 build number is `\u003e= 400`."
  ]
}
