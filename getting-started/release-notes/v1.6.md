...
- Fixed system monitor connection tracking to stop reporting `accept(2)` events.

### 🎁 Misc

- Improved `tcp_accept` handling to reduce stack usage while aligning with `tcp_connect`.
- Improved protocol detection by reading `sk_protocol` via BTF when available and falling back to an offset-based read otherwise.
- Updated Go module dependencies from Go 1.24.11 to Go 1.24.12.
