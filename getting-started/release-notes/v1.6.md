...
{
  "🎁 Misc": [
    "Improved how the system detects which runtime to use when connecting via a socket path. Specifically, `DetectRuntimeViaMap` now derives the runtime from the detected socket path by calling `determineRuntimeFromSocket` before returning."
  ]
}
