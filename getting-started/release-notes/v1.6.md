...
- **New Features:** Added support for running host-only workflows without starting Kubernetes (`k8s=false`) and for stamping USB log entries with timestamps for easier troubleshooting.
- **Misc:** Updated core dependencies to keep the toolchain and networking components current, including Go 1.24.13 and `github.com/cilium/cilium` v1.18.4.
