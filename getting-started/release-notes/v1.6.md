...
{
  "🎁 Misc": [
    "This release updates the KubeArmor Helm chart and corrects how the deployed workloads are configured so installations run reliably. Specifically, it bumps the Helm chart version and fixes the workloads’ YAML structure along with the CLI argument wiring and TLS-related flags used by the chart templates."
  ]
}
