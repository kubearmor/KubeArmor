...
- Added `allowedArgs` process-argument matching for process execution policies (BPF-LSM) to enable more precise policy scoping.
- Added support for applying process-argument matching to owner-only enforcement so ownership-based rules also consider how a process was invoked.

### 🐛 Bug Fixes

- Fixed network connect event argument capture to safely copy protocol names before use, avoiding invalid pointer reads.

### 🎁 Misc

- Updated internal Go code to modern idioms and replaced deprecated gRPC insecure client APIs.
- Updated build/release tooling and chart versions, including CodeQL action v4, refreshed Alpine images, and stable release v1.6.6.
- Improved CI posture test reliability by switching to an nginx test app and increasing waits and timeouts.
- Restricted special-case BPF compilation and macros to RHEL 9 kernels.
- Added runtime-configurable untracked namespace exclusions via a ConfigMap-backed global list.
- Added an option to omit the Resource field from process visibility logs via ConfigMap/CLI settings.
- Reduced BPF map key sizes by trimming path and source key buffers from 256 to 200 bytes.
- Skipped enforcer initialization when enforcement is disabled or LSM order is `none`, and propagated the effective setting through the operator.
- Refactored CLI and operator configuration into options structs for more consistent configuration handling.
- Hardened cleanup routines to avoid deleting empty/root paths and ensured SELinux profile directory config is loaded correctly.
- Updated copyright headers to 2026 and refreshed docs around BPF-LSM/kernel requirements and Vagrant examples.
