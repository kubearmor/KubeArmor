...
{
  "🐜 Bug Fixes": [
    "Fixed an issue where the container runtime interface socket address could be formatted incorrectly, which could prevent the runtime from being reached reliably. Specifically, the CRI socket URL formatting was corrected and the argument list is now updated in place so the final args slice matches the corrected socket value."
  ]
}
