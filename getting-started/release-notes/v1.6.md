...
- Added USB device observability to monitor attach/detach events and device attributes. (cc547521)
- Added USB Device Handler to enforce host USB device allow/deny via policies. (d5a17d66)
- Added eventData field to telemetry events for structured event payloads. (aa076a4b)
- Added support for unorchestrated rootful Podman using an OCI hook. (e8fc3936)

### Improvements

- Improved operator deployment by adding Helm values for environment variables and nodeSelector, plus support via the KubeArmorConfig CRD. (26caa65b, cd89425c)
- Bumped cilium/ebpf to resolve BTF parsing issues on some kernels. (2721379c)
- Updated presets to include an action field in generated alerts. (1efb18f4)
- Moved imagePullSecrets to the image object in Helm values for consistency. (62737962)
- Upgraded Go and related modules to address security advisories (gosec/govuln). (79083909, 94386b37, 916429fa, 306a2814)

### Bug Fixes

- Fixed missing action in syscall logs. (4eb295df)
- Fixed panic during controller nodeSelector updates. (7e91b9e7)
- Fixed image repository handling by adding the docker.io prefix for CRI-O 1.34 compatibility. (c8e52045)
