...
{
  "🆕 New Features": [
    "You can now see human-readable usernames in host logs and alerts to make investigations and triage easier. The system caches UID-to-username lookups and propagates usernames through internal types and protobuf APIs so alerts include the resolved username when available.",
    "You can now explicitly choose which container runtime socket to use when deploying and running the agent to avoid runtime auto-detection issues. A plumbed --socket-file option is passed from Helm through to snitch, with validation and tests, and runtime detection derives the runtime from the matched socket path via determineRuntimeFromSocket."
  ],
  "🎁 Misc": [
    "Deployment manifests now set clearer baseline resource consumption to improve scheduling and cluster stability. Resource requests and limits were added across relay, the KubeArmor controller, the DaemonSet, and snitch workloads.",
    "CI and test configuration has been updated so builds and runtime behavior are more consistent across environments. Helm/CI workflows were updated to use the correct kubearmor-operator image tag values key, and an LSM order flag was added to test setup and forwarded into KubeArmor execution.",
    "Developer run targets and logs have been improved to make local host-only runs easier to interpret. USB logs now include timestamps, Kubernetes mode is disabled in the host-only run target via k8s=false, and dependencies were bumped including Go to 1.24.13 and github.com/cilium/cilium to v1.18.4.",
    "Platform build logic has been clarified to avoid incorrect feature gating on RHEL 9 builds. The RHEL9_BUILD_GTE_400 macro is now defined only when the RHEL9 build number is \u003e= 400.",
    "Security scanning noise was reduced while keeping behavior unchanged to make builds easier to maintain. A Kubernetes secret field was renamed and gosec suppression comments were added around specific file checks and HTTP calls.",
    "Alert throttling is now safer under concurrent access to reduce the chance of intermittent alerting issues. A read-write mutex (RWMutex) was added to protect AlertMap and is now used consistently in alert-throttling operations.",
    "Release metadata has been updated to reflect the latest stable version. The stable release version was bumped from v1.6.6 to v1.6.8."
  ],
  "🐜 Bug Fixes": [
    "Host policy and endpoint updates are now more reliable under heavy concurrency to prevent intermittent failures and race conditions. The implementation hardens locking by copying slices under read locks, updating originals via keyed lookup under write locks, adding missing endpoint locking in a containerd update path, and expanding race-condition test coverage (with later test updates to randomize endpoint removal and shorten duration).",
    "Kubernetes DaemonSet generation now consistently applies the CRI socket argument so workloads start with the intended runtime configuration. CRI socket argument injection was moved to occur after init container configuration in generateDaemonset, ensuring the DaemonSet always sets the socket argument correctly."
  ]
}
