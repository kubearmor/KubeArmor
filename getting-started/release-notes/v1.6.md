...
{
  "🆕 New Features": [
    "Added safer cleanup behavior that prevents accidental deletion of important directories during teardown. Cleanup code paths now use safe remove helpers that refuse to delete empty strings or root paths instead of calling `os.Remove`/`os.RemoveAll` directly."
  ],
  "🎁 Misc": [
    "Made the SELinux profile directory configurable at startup so it is consistently loaded from configuration. `LoadConfig` now assigns `GlobalCfg.SELinuxProfileDir` from the `ConfigSELinuxProfileDir` Viper key."
  ]
}
