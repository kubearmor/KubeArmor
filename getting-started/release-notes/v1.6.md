...
<!-- BEGIN RELEASE NOTES: v1.6.6 (2026-01-20) -->
```json
{
  "🆕 New Features": [
    "You can now match process executions based on their command-line arguments, so policies can allow or deny a program more precisely than by path alone. This adds `allowedArgs`/`matchArgs` support end-to-end (config, CRDs, operator wiring, tests) and connects those fields into BPF-enforced exec rules via new/updated argument-aware BPF maps."
  ],
  "🎁 Misc": [
    "The system uses less memory for certain internal keys while keeping schema validation aligned with runtime constraints. This reduces InnerKey `Path` and `Source` byte array sizes from 256 to 200 (following earlier BPF buffer reductions) and aligns CRD `maxLength` limits with the updated BPF key/argument buffer sizes, alongside a controller-gen version bump and updated documentation of argument matching limits and usage."
  ],
  "🐜 Bug Fixes": [
    "Policies that use argument matching now enforce ownership and argument constraints more reliably and clean up state more safely. This refines the BPF owner/argument enforcement logic and argument storage, adjusts cleanup ordering, and improves cleanup-related logging to reduce leaks and improve diagnosability."
  ]
}

```
<!-- END RELEASE NOTES: v1.6.6 (2026-01-20) -->
