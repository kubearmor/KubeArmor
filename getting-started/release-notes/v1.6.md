...
### Breaking Changes
- Moved Helm Operator image pull secrets under the image object. Update chart values to use `.Values.kubearmorOperator.image.imagePullSecrets` (was `.Values.kubearmorOperator.imagePullSecrets`). See PR #2190.

### New Features
- Added USB Device Observability support to surface USB attach/detach events and metadata. (PR #2172)
- Added USB Device Enforcement with a new USB Device Handler to control device access. (PR #2194)
- Added structured event data to telemetry for richer context in alerts/logs. (PR #2204)
- Introduced File Integrity Monitoring (FIM) via Linux IMA: IMA hash visibility, dynamic (un)initialization, and module separation. (PR #2170 and follow-ups)
- Enabled OCI hook support for Podman in non‑Kubernetes environments (rootful/rootless), with a non‑blocking hook handler. (PR #1874)

### Improvements
- Helm: Added support for nodeSelector and env configuration via KubeArmorConfig for the operator. (PR #2187)
- Helm: Corrected tolerations indentation in the operator template. (PR #2138)
- Images: Added `docker.io` prefix for CRI‑O 1.34 compatibility. (commit c8e52045)

### Bug Fixes
- Fixed missing action in syscall logs. (PR #2169)
- Updated preset alerts to ensure action field was set correctly. (PR #2210)
- Fixed controller panic during nodeSelector updates. (PR #2193)

### Dependencies
- Bumped cilium/ebpf to address BTF parsing issues. (PR #2200)
- Upgraded Go toolchain and Docker package to remediate vulnerabilities. (commits 79083909, 94386b37)

### Documentation
- FAQ: Documented using grubby to enable BPF‑LSM on RHEL. (PR #2174)
- Testing Guide: Fixed test script paths. (commit 083dafce)

### Upgrade Notes
- Helm chart users must update values for image pull secrets to the new path: `.Values.kubearmorOperator.image.imagePullSecrets`.
- Telemetry consumers should account for the added structured event data fields when parsing alerts/logs.
