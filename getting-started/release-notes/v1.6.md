...
## 2026-02-10 Patch Updates (v1.6.6)
...
{
  "🆕 New Features": [
    "Host-only runs can now be executed without starting Kubernetes, making lightweight workflows easier to run and validate."
  ],
  "🎁 Misc": [
    "The project now builds and runs against updated core dependencies to keep the toolchain and networking components current."
  ]
}
...
```json
{
  ...
  "🎁 Misc": [
    "The project now builds and runs against updated core dependencies to keep the toolchain and networking components current. This updates Go to 1.24.13 and bumps `github.com/cilium/cilium` to v1.18.4."
  ]
}
...
{
  "🆕 New Features": [
    "Host-only runs can now be executed without starting Kubernetes, making lightweight workflows easier to run and validate. This adds a `k8s=false` flag to the host-only run path and also stamps USB log entries with timestamps for clearer sequencing during troubleshooting."
  ],
  "🎁 Misc": [
    "The project now builds and runs against updated core dependencies to keep the toolchain and networking components current. This updates Go to 1.24.13 and bumps `github.com/cilium/cilium` to v1.18.4."
  ]
