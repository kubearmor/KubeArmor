...
{
  "🆕 New Features": [
    "You can now configure the operator through a single, consolidated set of options instead of scattered settings. This introduces an `OperatorOptions` struct, refactors operator wiring to use it, and passes the selected LSM order to managed KubeArmor components via command-line arguments."
  ],
  "🎁 Misc": [
    "Snitch configuration is now simpler and more consistent by using one options object rather than multiple global variables. The snitch command configuration was refactored to use a unified `SnitchOptions` struct instead of separate globals."
  ],
  "🐜 Bug Fixes": [
    "Systems configured with no LSM ordering will no longer trigger incorrect LSM detection or errors. When the LSM order is set to `none`, detection is skipped and any unsupported LSM is treated as `none`."
  ]
}
