...
{
  "🎁 Misc": [
    "TCP accept handling now uses a more memory-efficient approach so it consumes less stack space while staying consistent with related connection tracking.",
    "Protocol detection is now more reliable on systems that support modern kernel type information, while still working on older builds.",
    "The project’s Go module dependencies were refreshed to the latest patch level to keep builds current."
  ],
  "🐜 Bug Fixes": [
    "The system monitor no longer tracks new incoming connections via the accept system call, reducing monitoring noise and overhead."
  ]
}
