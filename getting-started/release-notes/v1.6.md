...
## 2026-01-14 (Event Time)

<!--
NOTE: The release note entries for this update are injected below as structured JSON.
In a follow-up commit, this JSON will be rendered into the standard release notes sections:
- 🆕 New Features
- 🐛 Bug Fixes
- 🎁 Misc
...
{
  "🆕 New Features": [
    "You can now choose to omit the Kubernetes Resource value from process visibility logs when you do not want it included. This is controlled by the `dropResourceFromProcessLogs` / `DropResourceFromProcessLogs` boolean (default `false`) exposed via the CLI/config, the operator `KubeArmorConfig` spec (including Helm values/ConfigMap wiring), and it causes `pb.Log.Resource` to be left empty for `Operation == \"Process\"` when enabled (PR #2044)."
  ],
  "🎁 Misc": [
    "The KubeArmor Operator chart and configuration documentation were updated so operators can enable dropping the Resource field at deploy time. The Helm value `dropResourceFromProcessLogs` (default `false`) is documented and propagated through the operator-managed ConfigMap for dynamic updates (PR #2044)."
  ]
}
