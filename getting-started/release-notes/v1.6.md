...
**Title:** Patch update for KubeArmor v1.6  
**Date:** 2026-02-10  
**Version:** v1.6.6

{
  "🆕 New Features": [
    "Host activity logs now show human-readable usernames instead of only numeric user IDs, which makes investigations and audits easier to follow. The system adds a cached UID→username resolver for host logs and threads the resolved username through the internal log type and the protobuf Alert/Log message fields."
  ]
}
