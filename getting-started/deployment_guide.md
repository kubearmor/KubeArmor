...
### 4. Use cases and deploying Security Policy
...
MySQL uses `/var/lib/mysql` path as the default data directory. An audit policy will generate an alert for each operation associated with the resource i.e. a file or binary, specified in the policy. The details logs about the event can help in identifying unauthrized access and potential security breaches. We'll Create a KubeArmor Security policy to audit access to `/var/lib/mysql` folder path recursively.
...
Start Observing KubeArmor logs by executing the following command in a sperate terminal/tab.
```
$ karmor log
...
Before simulating the policy violation in the next step execute the following command in a new terminal/tab to start observing KubeArmor logs/alerts, output for the same will be discussed in upcoming section `Getting Alerts/Telemetry from KubeArmor`.
...
KubeArmor will generate an alert for the event that violates the applied security policy, the output will be similar as following:
...
We can see that after applying policy now KubeArmor is generating alert for all the access to the `/var/lib/mysql` directory.
...
A pod may contain unprotected files with sensitive data that need to be protected from unautherized access. In our sample wordpress app, wordpress pod contains a file `wp-config.php` that has sensitive auth credentials. KubeArmor security policy can be applied to block access to this file from unknown processes.
...
It can be seen in the following alert that, the process `/bin/cat` with pid as 418 was trying to access the file `wp-config.php` that leads to the policy violation so the access is blocked by the enforcer.
...
Conatiner base images having pacakage managers i.e. `apt` that is used to install packages required by the application at runtime. when a container shipped to the production it should be avoided to have package managers running inside the container, it increases the attack surface. If a container get compromised, attackers can used the package managers to install the binaries required to escalate the attack.

Our sample `wordpress-mysql` application container have package manager `apt` and can be executed once we're inside the container.
...
An information rich alert will be generated for every event/operation that violates the KubeArmor security policy. In the following alert we can see the information of the event that voilates the applied policy in the earlier step that block the execution of the processes `apt` and `apt-get`.
