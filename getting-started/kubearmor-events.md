...
| EventData              | structured key-value data derived from the log's `Data` field, and for `Network` operations also from the `Resource` field | {"Syscall":"SYS_CONNECT","Sin_port":"53"}                                                               |
