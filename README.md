...
### Nested Container Runtime Visibility (Docker-in-Docker)

When running Docker-in-Docker (DinD) workloads using containerd on
systems configured with cgroup v2, KubeArmor may not trace activities
originating from the inner container runtime.

This is a known limitation of nested container visibility under
cgroup v2 environments. As a result, security events generated by
processes inside inner containers may not be surfaced.
