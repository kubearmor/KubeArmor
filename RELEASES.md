...
## Cryptographic Signing & Build Provenance

All official release container images and binary packages generated by KubeArmor automated release workflows are cryptographically signed and provenance-verified per OpenSSF security best practices:
- **Container Image Signing:** Images built by release workflows are signed with [Sigstore Cosign](https://github.com/sigstore/cosign) using Keyless OIDC authentication tied to GitHub Actions workflows.
- **Binary & Archive Artifact Signing:** Release binaries (`.tar.gz`, `.deb`, `.rpm`) have associated `.sig`, `.pem`, and `.sigstore.json` signature bundles attached directly to GitHub Releases.
- **SLSA Level 3 Provenance:** Release assets include SLSA Level 3 build provenance (`multiple.intoto.jsonl`) generated automatically via [`slsa-framework/slsa-github-generator`](https://github.com/slsa-framework/slsa-github-generator).

Verification instructions for end-users and adopters are documented in [SECURITY.md#verifying-release-artifacts-and-container-images](./SECURITY.md#verifying-release-artifacts-and-container-images).
