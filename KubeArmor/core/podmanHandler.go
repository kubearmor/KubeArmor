	...
	if action == "create" {
		...
		if _, ok := dm.Containers[container.ContainerID]; !ok {
			...
			for _, secPol := range dm.SecurityPolicies {
				dm.Logger.Printf("Identities: %v, SuperIdentities: %v\n", secPol.Spec.Selector.Identities, endPoint.Identities)
		...
		if dm.SystemMonitor != nil && cfg.GlobalCfg.Policy {
			...
			dm.Logger.Printf("IF loop must conditions are  endPoint.SecurityPolicies%d  endPoint.PolicyEnabled %d tp.KubeArmorPolicyEnabled %d",len(endPoint.SecurityPolicies), endPoint.PolicyEnabled,tp.KubeArmorPolicyEnabled )
