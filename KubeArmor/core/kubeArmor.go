...
func (dm *KubeArmorDaemon) InitNetworkPolicyEnforcer() error {
	dm.NetworkPolicyEnforcer = ne.NewNetworkPolicyEnforcer(dm.Logger)
	if dm.NetworkPolicyEnforcer == nil {
		return fmt.Errorf("failed to create network policy enforcer")
	}
	return nil
}
...
func KubeArmor() {
	...
	if dm.K8sEnabled && cfg.GlobalCfg.NetworkPolicyEnforcer {
		// watch network security policies
		go dm.WatchNetworkSecurityPolicies(timeout)
	}

	if !dm.K8sEnabled && (enableContainerPolicy || cfg.GlobalCfg.HostPolicy || cfg.GlobalCfg.NetworkPolicyEnforcer) {
