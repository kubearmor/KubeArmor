...
func (p *Preset) AddContainerIDToMap(id string, ckv NsKey, action string) error {
	p.Logger.Printf("[ProtectProc] adding container with id to protectEnv_map exec map: %s\n", id)
	a := base.Block
	if action == "Audit" {
		a = base.Audit
	}
...
func (p *Preset) DeleteContainerIDFromMap(id string, ckv NsKey) error {
	p.Logger.Printf("[ProtectProc] deleting container with id to protectEnv_map exec map: %s\n", id)
