	...
	*/
	...
	if log.Type == "MatchedPolicy" || log.Type == "MatchedHostPolicy" || log.Type == "SystemEvent" {
		...
		if log.Type == "MatchedHostPolicy" {
			...
			pbAlert.UserName = fd.UserNameMap.GetUsername(uint32(log.UID))
		}
		...
		if log.Type == "HostLog" {
			// getting username is supported for host logs only due to performance overhead
			// TODO: Optimize username fetching for container logs
			pbLog.UserName = fd.UserNameMap.GetUsername(uint32(log.UID))
		}
