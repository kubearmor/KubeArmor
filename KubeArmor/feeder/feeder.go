...
func (fd *Feeder) PushLog(log tp.Log) {
	...
	*/
	...
	if log.Type == "MatchedPolicy" || log.Type == "MatchedHostPolicy" || log.Type == "SystemEvent" {
		...
		// checking throttling condition for "Audit" alerts when enforcer is 'eBPF Monitor' or 'Minifilter'
		if cfg.GlobalCfg.AlertThrottling && ((strings.Contains(log.Action, "Audit") && (log.Enforcer == "eBPF Monitor" || log.Enforcer == "Minifilter")) || (log.Type == "MatchedHostPolicy" && (log.Enforcer == "AppArmor" || log.Enforcer == "eBPF Monitor" || log.Enforcer == "Minifilter"))) {
