...
func (de *USBDeviceHandlerImpl) monitorUSBDeviceEvents() {
...
func (de *USBDeviceHandlerImpl) handleAlreadyConnectedDevies() {
...
func (de *USBDeviceHandlerImpl) handleDeviceAdd(sysPath, info string) {
...
func (de *USBDeviceHandlerImpl) handleDeviceRemove(sysPath, info string) {
	level := int32(getUSBLevel(filepath.Base(sysPath)))
	class, subclass, protocol, err := parseUSBCodes(info)
	if err != nil {
		de.Logger.Errf("Error: %v", err)
...
func (de *USBDeviceHandlerImpl) setDeviceAuthorization(rule EnforcementRule, sysPath string, config bool) {
...
func (de *USBDeviceHandlerImpl) generateLog(r EnforcementRule, sysPath string) {
	log := tp.Log{}
...
func (de *USBDeviceHandlerImpl) UpdateHostSecurityPolicies(secPolicies []tp.HostSecurityPolicy) {
