...
static long
get_task_filename(const struct trace_event_raw_sched_process_exec *ctx)
{
	long ret;
...
static long
get_task_ids(const struct trace_event_raw_sched_process_exec *ctx)
{
	long ret;
...
static u64
check_hash_elem(struct bpf_map *map, struct pattern_key *key,
		struct pattern_value *val, struct callback_ctx *data)
{
	if (!key || !val || !data)
		return 0;
...
static bool
task_auditable_1(const struct trace_event_raw_sched_process_exec *ctx)
{
...
int
ka_ea_sched_process_exec_1(struct trace_event_raw_sched_process_exec *ctx)
{
	if (!task_auditable_1(ctx))
		return 0;

	if (set_task_for_audit() < 0)
		bpf_printk("[ka-ea-process]: failure setting %s for audit",
			   filename);
	else
...
int
ka_ea_sched_process_exec_0(struct trace_event_raw_sched_process_exec *ctx)
{
	if (!task_auditable_0())
		return 0;
...
int
ka_ea_sched_process_exit(struct trace_event_raw_sched_process_template *ctx)
{
