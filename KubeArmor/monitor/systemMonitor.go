...
func (mon *SystemMonitor) UpdateMatchArgsConfig() {
	if mon.BpfConfigMap == nil { // no eBPF system monitor (e.g. non-Linux)
		return
	}
...
func (mon *SystemMonitor) UpdateVisibility() {
	if mon.BpfNsVisibilityMap == nil { // no eBPF system monitor (e.g. non-Linux)
		return
	}
...
func (mon *SystemMonitor) TraceSyscall() {
	if mon.SyscallPerfMap != nil {
		...
		// No perf buffer: the eBPF system monitor is not initialized (e.g. non-Linux,
		// or perf map setup failed - already logged by InitBPF). Nothing to trace.
		mon.Logger.Print("System monitor perf buffer not initialized; syscall tracing is disabled")
		return
	}
