...
type batchAuditPaths struct {
	Path   [batchAuditMaxPathLen]byte // file or exec path
	Source [batchAuditMaxPathLen]byte // source exec path
}

type batchAuditPolicyKey struct {
	Okey  NsKey
	Paths batchAuditPaths
}
...
func (mon *SystemMonitor) PollBatchAuditEvents() {
	...
	for {
		...
		wait := max(interval-time.Since(lastPoll), 0)
		...
		case <-mon.BatchAuditWakeChan:
			...
			// policy refreshes should not keep postponing pending flushes
			if time.Since(lastPoll) >= mon.minBatchAuditInterval() {
				mon.pollBatchAuditMapOnce()
				lastPoll = time.Now()
			}
		}
...
func (mon *SystemMonitor) applyBatchAuditPolicySpec(ns NsKey, spec batchAuditPolicySpec) error {
	...
	mon.BatchAuditStateLock.Lock()
	mon.BatchAuditPolicies[policyHash] = batchAuditPolicyMeta{
		Kind:            spec.Kind,
...
func batchAuditPolicyHash(kind, namespace, name string) (uint64, error) {
	h := fnv.New64a()
	_, err := h.Write([]byte(kind + "/" + namespace + "/" + name))
	if err != nil {
		return 0, err
	}
	return h.Sum64(), nil
...
func (mon *SystemMonitor) updateBatchAuditPolicyRule(ns NsKey, path, source string, processMask, fileMask uint16, policyHash uint64) {
	...
	// process and file rules can share the same map key. Merge masks for the
	// same policy to avoid one rule overwriting the other
...
func decodeBatchAuditSample(raw []byte) (batchAuditDecodedSample, error) {
	var out batchAuditDecodedSample
...
func formatBatchAuditEvent(log *tp.Log, entry batchAuditDecodedSample, ret *batchAuditDecodedSample) error {
	...
	case SysOpen:
		...
		log.Operation = "File"
		log.Resource = path
		log.Data = fmt.Sprintf("syscall=%s flags=%s", GetSyscallName(entry.Ctx.EventID), flags)
	case SysOpenAt:
		if len(entry.Args) != 3 {
		...
		log.Operation = "File"
		log.Resource = path
		log.Data = fmt.Sprintf("syscall=%s fd=%d flags=%s", GetSyscallName(entry.Ctx.EventID), fd, flags)
	case SysUnlink:
		if len(entry.Args) != 2 {
		...
		log.Operation = "File"
		log.Resource = path
		log.Data = fmt.Sprintf("syscall=%s", GetSyscallName(entry.Ctx.EventID))
	case SysUnlinkAt:
		if len(entry.Args) != 3 {
	...
	case SysChown:
		...
		log.Operation = "File"
		log.Resource = path
		log.Data = fmt.Sprintf("syscall=%s userid=%d group=%d", GetSyscallName(entry.Ctx.EventID), uid, gid)
	case SysFChownAt:
		if len(entry.Args) != 5 {
		...
		log.Operation = "File"
		log.Resource = path
		log.Data = fmt.Sprintf("syscall=%s userid=%d group=%d mode=%d", GetSyscallName(entry.Ctx.EventID), uid, gid, mode)
	case SysExecve:
		if len(entry.Args) != 2 {
	...
	case SysExecveAt:
		...
		log.Data = fmt.Sprintf("syscall=%s fd=%d flag=%s", GetSyscallName(entry.Ctx.EventID), fd, procExecFlag)
	default:
