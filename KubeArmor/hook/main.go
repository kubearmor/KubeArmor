...
func main() {
	...
	flag.StringVar(&runtimeSocket, "runtime-socket", "/run/podman/podman.sock", "container runtime socket")
	...
	if err != nil {
		log.Println("failed to marshal state: %v", err)
		os.Exit(1)
	}
	...
	_, err = logFile.WriteString("Container State is:" + string(stateJSON) + "\n")
	if err != nil {
		log.Println("failed to write to log file: %v", err)
		os.Exit(1)
	}
...
func run(state specs.State) error {
	...
	// Get the HOME environment variable to get information on Podman rootless containers
	logFile, err := os.OpenFile(LOGPATH, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	_, err = logFile.WriteString("BEFORE SUDO_USER "  + "\n")

	// sudoUser := os.Getenv("SUDO_USER")
	// if sudoUser == "" {
	// 	log.Fatal("SUDO_USER environment variable is not set")
	// 	_, err = logFile.WriteString("SUDO_USER environment variable is not set: "+"\n")
	// }
    ...
    _, err = logFile.WriteString("Found user with home directory: " + homeDir + "\n")
    rootlessContainersPath := filepath.Join(homeDir, ".local/share/containers/storage/overlay-containers")
    _, err = logFile.WriteString("rootlessContainersPath: " + rootlessContainersPath + "\n")
	...
	_, err = logFile.WriteString(" after rootful: " +"\n")
	...
	_, err = logFile.WriteString(" after rootless: " +"\n")


	var paths []string

	isRootFullPodman := runtimeSocket == "unix:///run/podman/podman.sock"
	_, err = logFile.WriteString(" check socket: " +runtimeSocket+"\n")

	if isRootFullPodman {
	    paths = []string{metadataPath1, metadataPath2}
		_, err = logFile.WriteString(" in rootfull path: " +"\n")
	} else {
		_, err = logFile.WriteString(" in rootless path: " +"\n")
		paths = []string{metadataPath3, metadataPath4}
	}
	_, err = logFile.WriteString("AFTER SUDO_USER: " +"\n")
...
func fetchContainerDetails(containerID, metadataPath string) (MetadataDetails, error) {
	...
	for _, container := range containers {
		...
		if err == nil {
			// Log the container info to the file
			_, err = logFile.WriteString("Container from path: " + string(containerJSON) + "\n")
			if err != nil {
...
func sendContainer(container types.Container, operation types.HookOperation) error {
	...
	// Write data to the log file
	_, err = logFile.WriteString(string(dataJSON) + "\n")
	if err != nil {
