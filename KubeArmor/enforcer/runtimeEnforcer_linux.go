...
type RuntimeEnforcerLinux struct {
	// logger
	Logger *fd.Feeder
...
// selectLsm Function
func selectLsm(re *RuntimeEnforcerLinux, lsmOrder, availablelsms, supportedlsm []string, node tp.Node, pinpath string, logger *fd.Feeder, monitor *mon.SystemMonitor) *RuntimeEnforcerLinux {
	var err error
	var lsm string
...
bpf:
	...
	if re.bpfEnforcer != nil {
		...
		if err := monitor.UpdateConfiguration(uint32(2), uint32(1)); err != nil {
...
probeBPFLSM:
	...
	return selectLsm(re, cfg.GlobalCfg.LsmOrder, availablelsms, lsms, node, kl.GetMapRoot(), logger, monitor)
}

func (re *RuntimeEnforcerLinux) GetEnforcerType() string {
	return re.EnforcerType
}

// RegisterContainer registers container identifiers to BPFEnforcer Map
func (re *RuntimeEnforcerLinux) RegisterContainer(containerID string, pidns, mntns uint32) {
...
func (re *RuntimeEnforcerLinux) UnregisterContainer(containerID string) {
	// skip if runtime enforcer is not active
	if re == nil {
		return
	}
...
func (re *RuntimeEnforcerLinux) UpdateAppArmorProfiles(podName string, action string, profiles map[string]string, privilegedProfiles map[string]struct{}) {
	// skip if runtime enforcer is not active
	if re == nil {
		return
	}
...
func (re *RuntimeEnforcerLinux) UpdateSecurityPolicies(endPoint tp.EndPoint) {
	// skip if runtime enforcer is not active
	if re == nil {
		return
	}
...
func (re *RuntimeEnforcerLinux) UpdateHostSecurityPolicies(secPolicies []tp.HostSecurityPolicy) {
	// skip if runtime enforcer is not active
	if re == nil {
		return
	}
...
func (re *RuntimeEnforcerLinux) DestroyRuntimeEnforcer() error {
	// skip if runtime enforcer is not active
	if re == nil {
		return nil
	}
