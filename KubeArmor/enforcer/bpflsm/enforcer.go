...
func updateSocketEventLog(log tp.Log, eventID int32, data InnerKey) tp.Log {
	log.Operation = "Network"

	switch data.Path[0] {
	case byte(FAMILY):
		log.Resource = "domain=" + mon.GetSocketDomain(uint32(data.Path[1]))
	case byte(TYPE):
		log.Resource = fd.GetProtocolFromType(int32(data.Path[1]))
	case byte(PROTOCOL):
		log.Resource = fd.GetProtocolFromName(mon.GetProtocol(int32(data.Path[1])))
	}

	log.Data = "lsm=" + mon.GetSyscallName(eventID) + " " + log.Resource
	return log
}
...
func (be *BPFEnforcer) TraceEvents() {
	...
	for {
		...
		case mon.SocketCreate, mon.SocketConnect, mon.SocketAccept:
			log = updateSocketEventLog(log, event.EventID, event.Data)
